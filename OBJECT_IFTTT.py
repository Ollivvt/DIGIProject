...
SHEETS_event_name = 'DIGI_SHEETS'
SHEETS_key = 'RLoJsN4GIsV84Rmq6Y7fJ'
  ...
  while (key_detect==0) :
    ...
    if (times==1):
      ...
      for num in range(len(results)) :
        ...
        if (labels[label_id] == 'bottle') and (box_left > 320):    #框選特定類別
          ...
          r = requests.post(LINE_URL, params={"value1":labels[label_id],"value2":round(results[num]['score'],4),"value3":'JACK'})

          SHEETS_URL='https://maker.ifttt.com/trigger/' + SHEETS_event_name + '/with/key/' + SHEETS_key
          r = requests.post(SHEETS_URL, params={"value1":labels[label_id],"value2":round(results[num]['score'],4),"value3":'JACK'})
